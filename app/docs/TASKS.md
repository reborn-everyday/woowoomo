...
> **참조 순서**: 스코프 판단 → `docs/PRD.md`, 코딩 규칙 → `docs/AGENTS.md`, 실행 순서 → 이 파일
>
...
> **Competition override**: 이번 대회 본편에서는 Video File, local-video, YouTube 입력 준비, `ffprobe`/`ffmpeg` 처리 경로를 critical path에서 제외한다. 관련 문구는 deferred/simulated capability disclosure로만 유지하고, launch sequence는 live-safe 또는 seed/mock 검증에만 의존한다.

**반드시 먼저 수행:**

1. macOS 시스템 환경설정 → 개인 정보 보호 → **화면 기록** → 앱 허용
2. macOS 시스템 환경설정 → 개인 정보 보호 → **손쉬운 사용** → 앱 허용
3. Anthropic Console에서 Claude API key 준비
4. (선택) 조코딩 YouTube/local-video asset을 reference로 열어둘 수 있으나, 이는 disclosure/rehearsal 메모 용도일 뿐 launch blocker가 아니다

> ⚠️ 권한과 API key 사전 준비가 없으면 T7, T8이 실행 중 막힐 수 있다. video reference asset은 본편 blocker가 아니다.
...
```text
docs/PRD.md, docs/AGENTS.md, docs/TASKS.md 세 파일을 읽어.
...
| 빌드 에러 멈춤 | "에러 로그 읽고 docs/AGENTS.md 컨벤션 지키면서 수정해" |
...
- `src/shared/types.ts` — 공용 TypeScript 인터페이스:
  - **`AIProvider` interface**
    ...
    ```
  - `VisionResponse`: current_task, tool_in_video (legacy/disclosure field name), category, focus_score, task_state, notes
...
**QA**:
```text
1. analyzeScreenshot([일반 작업 화면 예시 이미지]) → tool_in_video 필드 포함
2. 작업 맥락이 없는 placeholder/loading-state 이미지 → focus_score: 0
