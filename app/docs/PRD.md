...
> 코딩 규칙은 `docs/AGENTS.md`, 실행 태스크는 `docs/TASKS.md` 참조.
...
제품 비전상 두 가지 입력 모드를 상정한다. 다만 Ralphthon 본편에서는 `Live Capture`만 competition critical path에 포함하고, `Video File`은 deferred 또는 simulated capability로만 문서화한다.

| 모드 | 입력 | 캡처 방식 | Window Watcher |
|------|------|----------|----------------|
| **Live Capture** | 사용자의 실시간 데스크톱 화면 | `record screen --screenshot` (설정 주기마다) | 활성 |
| **Video File** | 녹화된 영상 파일 (.mp4, .mov 등) | `ffmpeg` 기반 설계만 유지, 대회 본편에서는 deferred/simulated | 비활성 (영상에서 추출 불가) |

**Video File capability 메모, deferred/simulated:**
1. 이번 대회 본편에서는 실제 구현, acceptance, launch gate 대상에 포함하지 않는다.
2. local video, `ffprobe`, `ffmpeg` 흐름은 post-competition 구현 후보 또는 mock/simulation disclosure 용도로만 남긴다.
3. 관련 언급이 필요하면 `docs/SIMULATION_REGISTER.md`에 deferred capability로 공개한다.

---

## 2-1. 데모 환경 정의 (Ralphthon 전용)

> **이 섹션은 Ralphthon 시연 맥락 설명용 reference다.**
> **이번 대회 본편에서는 비디오 구현을 critical path에서 제외하며, YouTube/local-video 맥락은 simulation/mock 또는 deferred capability disclosure로만 사용한다.**

### 데모 화면 구성
- 본편 주 시연 경로: Live Capture 또는 seed/mock evidence 기반 흐름
- 조코딩 YouTube/local-video 자산은 deferred capability 설명이 필요할 때만 reference asset으로 사용
- 분석 대상 서사는 동일하게 **사용자가 어떤 작업을 하고 있었는지** 설명하는 데 둔다

### 데이터 소스 전략

legacy reference scenario에서는 Window Watcher의 앱 이름("Chrome")은 고정이지만 **탭 전환 시 윈도우 타이틀이 변한다.** Chrome 내 탭 전환은 Window Watcher로 감지 가능하다. 단, 영상 탭에서 벗어나지 않는 구간의 **영상 안 작업 전환**은 Vision으로 보완한다.
...
- **앱 활동 추적 (Window Watcher)**: 30초마다 현재 포커스 앱 이름 + 윈도우 타이틀 기록
  ...
  - **Deferred Video File capability에서는 비활성** (영상에서 앱 타이틀 추출 불가)
- **스크린샷/프레임 분석 (Vision)**: 설정 가능한 주기(기본 5분, 범위 1~15분)마다 이미지 → Claude Vision 분석
  ...
  - **Video File capability (deferred/simulated)**: `ffmpeg` 프레임 추출 설계만 문서화하고, 이번 대회 본편 acceptance에는 포함하지 않음
...
- **Context Switching 감지**:
  ...
  - Deferred Video capability를 나중에 활성화하면 Vision category 변화만 단일 소스로 사용
...
src/
  main/                      # Electron main process (Node.js)
    ...
    video-processor.ts       # deferred video path용 ffmpeg wrapper 설계 메모
  ...
  renderer/                  # React
    components/
      ...
      VideoImport.tsx         # deferred video path용 영상 임포트 UI 메모
    pages/
      report.tsx
      settings.tsx
  shared/
...
#### Video File capability, deferred/simulated
```text
본편 competition critical path 밖의 설계 메모:
  1. ffprobe로 영상 길이 조회:
...
이 경로는 이번 대회 본편 acceptance에 포함하지 않으며, 필요 시 disclosure 또는 post-competition 구현 메모로만 사용한다.
```
...
| `video:import` | deferred video path용 영상 파일 임포트 메모 |
| `video:progress` | deferred video path 진행률 메모 |
| `video:check-ffmpeg` | deferred video path용 ffmpeg 확인 메모 |
...
- **외부 CLI 사전 설치 필수**:
  ...
  - `ffmpeg`: `brew install ffmpeg` — deferred Video File path에서만 필요, 이번 대회 본편 launch blocker는 아님
  - 앱 시작 시 `record` 설치 확인은 본편 기준으로 수행한다. `ffmpeg` 확인과 안내는 deferred capability disclosure 범위로만 둔다.
...
> Note: Video File, local-video, `ffprobe`, `ffmpeg` 경로는 이번 대회 본편 acceptance에서 제외한다. 필요 시 deferred/simulated capability로만 공개한다.
...
| reference 영상 내 화면 인식 실패 | deferred capability reference에만 영향, 본편은 seed/mock 또는 live capture 설명으로 진행 |
| Window Watcher 타이틀 고정 구간 | Vision category 변화로 context switch 감지 보완 |
| reference 영상 1시간 → 스냅샷 12개뿐 | disclosure용 메모에만 반영하고 본편은 seed 데이터로 보완 |
...
| `ffmpeg` 미설치 | deferred Video File capability에만 영향, 본편 launch blocker 아님 |
| 영상 파일이 너무 큼 (수 시간) | deferred capability rehearsal에서만 검토, 본편 진행은 막지 않음 |
| ffmpeg 프레임 추출 실패 | deferred capability disclosure로 남기고 본편은 seed/mock path 유지 |
